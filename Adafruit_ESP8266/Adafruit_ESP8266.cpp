...
Adafruit_ESP8266::Adafruit_ESP8266(Stream *s, Stream *d, int8_t r) :
 //stream(s), debug(d), reset_pin(r), host(NULL), writing(false) {
  stream(s), debug(d), reset_pin(r), writing(false) {
  setTimeouts();
};
...
boolean Adafruit_ESP8266::connectToAP(const char *ssid, const char *pass) {
  char buf[256];
...
/**boolean Adafruit_ESP8266::connectTCP(Fstr *h, int port) {
  ...
  if(find(F("CONNECT")) || find(F("ALREADY CONNECT"))) {
    host = h;
    return true;
  }
  return false;
...
boolean Adafruit_ESP8266::connectTCP(const char *h, int port) {
 ...
 */
//boolean Adafruit_ESP8266::httpPost(const char *host, const char* uri,const char* data)
boolean Adafruit_ESP8266::httpPost(const char *host, const char* uri,const char* data)

{
  ...
  //String content_length_buffer = "Content-Length: ";
  //content_length_buffer+=strDataLength;
	char content_length_buffer[25] = "Content-Length: ";
	strcat(content_length_buffer,strDataLength);
	
	char buffer[200] = "POST "; 

  // if your post request is longer than 199 bytes/characters then increase this value
  //change to 100 if using sd_card
  ...
  buffer+=content_length_buffer.c_str();
  buffer+="\r\n";
  buffer+=data;**/
  strcat_P(buffer,uri);
